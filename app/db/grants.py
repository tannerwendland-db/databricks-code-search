...
write access (INSERT/UPDATE/DELETE + sequence usage) but no DDL. Mapping a
Databricks client-id to a concrete role is deferred to issue #6.
"""
...
def build_job_grants(schema: str, job_role: str) -> list[str]:
    ...
    return [
        f"GRANT USAGE ON SCHEMA {s} TO {r}",
        f"GRANT INSERT, UPDATE, DELETE ON ALL TABLES IN SCHEMA {s} TO {r}",
        f"GRANT USAGE ON ALL SEQUENCES IN SCHEMA {s} TO {r}",
        f"ALTER DEFAULT PRIVILEGES IN SCHEMA {s} GRANT INSERT, UPDATE, DELETE ON TABLES TO {r}",
