...
implementation, backed by a Databricks model-serving endpoint via
``databricks-sdk``. The SDK import is LAZY (inside the function body, not
module scope) so the flag-off path and unit tests never import it -- mirrors
...
def _query_batch(
    client: Any, endpoint: str, batch: list[str], *, timeout: float, max_retries: int
) -> list[list[float]]:
    """Query one batch, retrying up to ``max_retries`` times (small, bounded)."""
    last_exc: Exception | None = None
    for _attempt in range(max_retries + 1):
        try:
            response = client.serving_endpoints.query(name=endpoint, input=batch, timeout=timeout)
...
def databricks_embedder(
    ...
    *,
    ...
    """Build an :data:`EmbedFn` backed by the ``endpoint`` Databricks serving endpoint.

    Batches ``texts`` by ``batch_size``, queries each batch with a bounded
    retry, and asserts every returned vector's length equals ``dim`` before
    returning -- a mismatch (e.g. a model swap to a different dim) raises
    :class:`EmbeddingDimMismatchError` instead of writing a bad vector.

    ``client`` is a test seam: when provided, ``databricks.sdk`` is never
    imported (the fake stands in for a ``WorkspaceClient``).
    """
    if client is None:
    ...
    def embed(texts: list[str]) -> list[list[float]]:
        ...
        for i in range(0, len(texts), batch_size):
            ...
            vectors.extend(
                _query_batch(client, endpoint, batch, timeout=timeout, max_retries=max_retries)
            )
