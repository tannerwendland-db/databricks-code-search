...
| `branches.py` | Resolves a connection's branch globs against a repo's real branch list via `fnmatchcase`. Default branch always included and always first; empty globs means default-branch-only with no GitHub branches API call. `SOFT_BRANCH_CAP = 20` truncates (loud warning, not a failure); no override flag — fix the config. |
