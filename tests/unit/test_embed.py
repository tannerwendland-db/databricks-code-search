...
class _FakeServingEndpoints:
    ...
    def query(self, *, name: str, input: list[str], timeout: float) -> _FakeResponse:
        self.batches.append(list(input))
        return _FakeResponse(self._vectors_fn(input))


class _FakeClient:
    def __init__(self, vectors_fn: Any) -> None:
        self.serving_endpoints = _FakeServingEndpoints(vectors_fn)


@pytest.mark.unit
def test_batching_splits_by_batch_size() -> None:
    client = _FakeClient(lambda texts: [[0.0, 0.0] for _ in texts])
    embed = databricks_embedder("ep", client=client, dim=2, batch_size=2)
    vectors = embed(["a", "b", "c", "d", "e"])
    assert len(vectors) == 5
    assert client.serving_endpoints.batches == [["a", "b"], ["c", "d"], ["e"]]


@pytest.mark.unit
def test_single_batch_when_under_batch_size() -> None:
    client = _FakeClient(lambda texts: [[0.0] for _ in texts])
    embed = databricks_embedder("ep", client=client, dim=1, batch_size=10)
    embed(["a", "b"])
    assert client.serving_endpoints.batches == [["a", "b"]]


@pytest.mark.unit
def test_dim_mismatch_raises() -> None:
    client = _FakeClient(lambda texts: [[0.0, 0.0, 0.0] for _ in texts])  # dim 3, expect 2
    embed = databricks_embedder("ep", client=client, dim=2)
...
def test_retries_then_succeeds() -> None:
    ...
    embed = databricks_embedder("ep", client=client, dim=2, max_retries=2)
...
def test_retries_exhausted_reraises() -> None:
    ...
    embed = databricks_embedder("ep", client=client, dim=2, max_retries=1)
...
def test_short_batch_raises_count_mismatch() -> None:
    ...
    embed = databricks_embedder("ep", client=client, dim=2, batch_size=10)
...
def test_count_mismatch_is_not_retried() -> None:
    ...
    embed = databricks_embedder("ep", client=client, dim=2, batch_size=10, max_retries=3)
