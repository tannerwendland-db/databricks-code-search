...
def test_deletes_by_file_id_then_inserts_all_rows() -> None:
    ...
    written = write_chunks(
        conn,
        file_id=7,
        chunks=[(0, "chunk a", [0.1, 0.2]), (1, "chunk b", [0.3, 0.4])],
    )
    assert written == 2
    ...
    assert values == [
        {"file_id": 7, "chunk_index": 0, "content": "chunk a", "embedding": [0.1, 0.2]},
        {"file_id": 7, "chunk_index": 1, "content": "chunk b", "embedding": [0.3, 0.4]},
    ]
...
def test_ts_column_is_never_written() -> None:
    conn = _FakeConn()
    write_chunks(conn, file_id=1, chunks=[(0, "x", [0.0])])
